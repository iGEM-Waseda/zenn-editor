...
const preHandler = function (str, beg) {
...
const postHandler = function (str, end) {
...
function mdKatex(md) {
  for (const rule of inlineRules) {
    ...
    md.renderer.rules[rule.name] = (tokens, idx) => rule.tmpl.replace(/\$1/, md.utils.escapeHtml(tokens[idx].content));
  }
  for (const rule of blockRules) {
    md.block.ruler.before('fence', rule.name, function block(state, begLine, endLine, silent) {
      ...
      if (res && !silent && match) {
        ...
        let curline;
        for (curline = begLine; curline < endLine; curline++) if (endpos >= state.bMarks[curline] + state.tShift[curline] && endpos <= state.eMarks[curline])
